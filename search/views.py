    ...
    if form.is_valid():
        ...
        if 'searching' in request.POST.keys():
            if  not file_title and not file_content and not file_name and not file_author:
                form.add_error('file_title', 'Enter at least one property for searching')
        ...
        for result_one in content_result_list:
            ...
            result_one_obj_content = result_one['_source']['content'][int(content_keyword_index-15 if content_keyword_index>=15 else content_keyword_index):
                                                                      int(content_keyword_index+15 if content_keyword_index+15 < content_length else content_length-content_keyword_index)]
